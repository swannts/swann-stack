...
\n\nFeel free to reach out directly for hiring opportunities!`;
  ...
  if (q.includes('skills') || q.includes('languages') || q.includes('tech') || q.includes('stack') || q.includes('database') || q.includes('framework')) {
    return `### 🛠️ Swann's Technical Skill Matrix
...
Swann specializes in designing advanced AI agents and robust retrieval pipelines:
...
Swann has 5+ years of experience leading teams and building SaaS platforms:
\n\n${jobs}`;
  }
...
Swann has engineered several high-performance distributed platforms:
\n\n${projects}`;
  }
...
Swann excels in constructing highly scalable cloud-native architectures:
...
*   **API Gateways & Caching:** Scaled microservices with Nginx routers, GraphQL endpoints, and real-time state synchronization using WebSockets and Redis caching.`;
  ...
  return `### Hello! I am the "Ask Swann AI" Assistant 🤖
I am currently operating in **Local Grounded Mock Mode** because no external Groq API key is configured. However, I can answer any questions about Swann's background directly from his verified CV profile!

Swann (Abu Bokor Siddik) is a **Senior Full Stack Architect & AI Engineer** with 5+ years of experience specializing in:
...
*Try asking me about "skills", "experience", "projects", "AI background", or "how to contact Swann".*`;
}
