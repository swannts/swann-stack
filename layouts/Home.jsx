...
const HomeLayout = ({ main = {}, cta = {}, achievements = [] }) => {
  useEffect(() => {
    const handleAskAiClick = (e) => {
      e.preventDefault()
      window.dispatchEvent(new CustomEvent('open-swann-chat'))
    }
    const heroBtn = document.getElementById('hero-ask-ai')
    if (heroBtn) heroBtn.addEventListener('click', handleAskAiClick)
    return () => {
      if (heroBtn) heroBtn.removeEventListener('click', handleAskAiClick)
    }
  }, [])
